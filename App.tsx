...
const App: React.FC = () => {
  ...
  }, [setSessions, setActiveSessionId, currentUser]);
  ...
  const handleDeleteSession = (id: string) => {
    setModal({
      title: 'Delete Chat?',
      message: 'This will permanently delete this chat. This action cannot be undone.',
      onConfirm: () => {
  ...
  const handleArchiveSession = (id: string, archive: boolean) => {
    setSessions(prev => prev.map(s => s.id === id ? { ...s, isArchived: archive } : s));
    if (archive && id === (activeSessionId || '')) {
        setActiveSessionId(null);
    }
    // If we are unarchiving, switch back to the active chats view
    if (!archive) {
